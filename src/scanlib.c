...
#include "scanner.c"	/* for type scanners */
#include "scanner_callbacks.c"
...
void scanlib_print ( scanlib * scanlib ) {
    if(scanlib == NULL) return;
...
scannerserver * scanlib_get_scanner ( const char * name, scanlib * scanlib ) {
    if( scanlib == NULL || name == NULL ) return NULL;
...
scannerserver * scanlib_get_next_scanner ( scanlib * slib, scannerserver * s) {
    if(slib == NULL) {
	return NULL;
    }
    if(s==NULL) {
...
scannerserver * scanlib_new_scanner ( scanlib * scanlib ) {
    if( scanlib == NULL ) return NULL;
...
int scanlib_rm_scanner ( scanlib * scanlib, scannerserver * rmthis ) {
    /* LINKED LIST VERSION */
    /* remove from linked list */
    if(scanlib == NULL) return 0;
...
static int scanlib_scandir_filter ( const struct dirent * de ) {
    if( de == NULL ) return -1;		/* TODO: not sure what FAILURE value should be
					   as this is a callback used by 'scandir'
					 */
    return !fnmatch("*.so", de->d_name,0);
}

static void *wrap_dlsym (void * dl, const char * symbol, int * errc) {
    if( dl == NULL || symbol == NULL || errc == NULL )
	return NULL;
...
scanlib * scanlib_load_from_dir ( char * fpath, scanlib * scanlib) {
	...
	while (n--) {
		...
		*chop_point='\0';
		...
		case 0:
		    ...
		    newscanner->op->parse
			= (int (*)(void *))wrap_dlsym(dl, "hiena_scannerop_parse",
				&dlerrcount);
			...
			= (int (*)(yyscan_t *, void *))wrap_dlsym(dl, "yyparse",
				&dlerrcount);

		    dlerror();
		    newscanner->op->yylex_init_extra
			= (int (*)(void *, yyscan_t *))wrap_dlsym(dl, "yylex_init_extra",
				&dlerrcount);
...
scanlib * scanlib_load_from_path ( const char * scannerpath_str ) {
    ...
    // init scanner callbacks
    Hscb * cb = scanner_callbacks_init();
    if( cb == NULL ) {
	scanlib_cleanup( slib_scanners );
	return NULL;
    }
    slib_scanners->cb = cb;
    ...
    /* LOAD SCANNERS */
    /* look through scannerpath_str locations
       to find scanner so files.
    */
    char *pathstr = strdup(scannerpath_str);
    ...
    */
	...
	if ( stat(newstring, &sbuf) != -1 ) {
	    scanlib_load_from_dir(newstring, slib_scanners);
	}
	// cleanup
	free(newstring);
	*/
	...
	if ( stat(current_path, &sbuf) != -1 ) {
	    scanlib_load_from_dir(current_path, slib_scanners);
	}
...
scanlib *
cosmosfs_slib_scanners_init(const char * scannerpath_str ) {
    return scanlib_load_from_path( scannerpath_str );
}
